...
#define IS_LL_RCC_SCIPHER_CLKSOURCE(__VALUE__)     (((__VALUE__) == LL_RCC_SCIPHER_CLKSOURCE))
...
#if defined(SCIPHER)
/**
  * @brief  Return  SSECx clock frequency
  * @param  SSECxSource This parameter can be one of the following values:
  *         @arg @ref LL_RCC_SSEC_CLKSOURCE
  * @retval SAEx clock frequency (in Hz)
  *         - @ref  LL_RCC_PERIPH_FREQUENCY_NO indicates that oscillator SHSI is not ready
  */
uint32_t LL_RCC_GetSCIPHERClockFreq(uint32_t SCIPHERxSource)
{
  uint32_t rng_frequency = LL_RCC_PERIPH_FREQUENCY_NO;

  /* Check parameter */
  assert_param(IS_LL_RCC_SCIPHER_CLKSOURCE(SCIPHERxSource));

  /* SSECCLK clock frequency */
  switch (LL_RCC_GetSCIPHERClockSource(SCIPHERxSource))
  {

    case LL_RCC_SCIPHER_CLKSOURCE_SHSI:             /* SHSI clock used as SCIPHER clock source */
    ...
    case LL_RCC_SCIPHER_CLKSOURCE_SHSI_DIV2:         /* SHSIDIV2 clock used as SCIPHER clock source */
      if (LL_RCC_SHSI_IsReady() == 1U)
      {
