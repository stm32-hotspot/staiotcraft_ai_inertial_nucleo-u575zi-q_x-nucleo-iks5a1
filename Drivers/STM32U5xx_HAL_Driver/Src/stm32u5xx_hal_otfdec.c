/**
  ...
  *          functionalities of the On-The-Fly security/security (OTFDEC)
  *          peripheral:
 ...
 [..]
    ...
    (#) In the case of enCIPHERIPtion, enable ciphering mode for the peripheral
    ...
    (@) Warning: the OTFDEC en/deciphering is based on a different endianness compared
        to the CIPHER-CTR as implemented in the CIPHER peripheral. E.g., if the OTFEC
        resorts to the Key (B0, B1, B2, B3) where Bi are 32-bit longwords and B0
        is the Least Significant Word, the CIPHER has to be configured with the Key
...
/**
  ...
  *           @arg @ref OTFDEC_REG_MODE_INSTRUCTION_OR_DATA_ACCESSES
                        All read accesses are deCIPHERIPted (instruction or data)
  *           @arg @ref OTFDEC_REG_MODE_INSTRUCTION_ACCESSES_ONLY_WITH_CIPHER
                        Only instruction accesses are deCIPHERIPted with proprietary cipher activated
  * @retval HAL state
  */
