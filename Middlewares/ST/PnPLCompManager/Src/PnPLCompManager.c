...
static char global_uuid[PNPL_DEVICE_UUID_STRING_SIZE]; // UUID: 8 + "-" + 4 + "-" + 4 + "-" + 4 + "-" 12 = 36char + \0

static volatile const pnpl_device_uuid_t device_uuid = {
  .pnpl_device_uuid = PNPL_DEVICE_UUID_STRING,
  .pnpl_device_uuid_default = PNPL_DEVICE_UUID_STRING  
};
...
/* Get Device UUID string. If not replaced at deploy time, this returns the default placeholder. */
const char *PnPLGetDeviceUUID(void)
{
  return (const char*)device_uuid.pnpl_device_uuid;
}

/* Check if Device UUID has been replaced with a valid deployed value. */
bool PnPLHasValidDeviceUUID(void)
{
  return (strncmp((const char*)device_uuid.pnpl_device_uuid, (const char*)device_uuid.pnpl_device_uuid_default, PNPL_DEVICE_UUID_STRING_SIZE) != 0);
}
...
uint8_t PnPLGetPresentationJSON(char **serializedJSON, uint32_t *size)
{
  ...
  if (PnPLHasValidDeviceUUID())
  {
    (void)json_object_dotset_string(JSON_GetPresentation, "device_uuid", PnPLGetDeviceUUID());
  }
...
uint8_t PnPLGetDeviceStatusJSON(char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  /*
      "schema_version": "3.0.0",                 (Reference to the schema version adopted.)
  ...
  */
  ...
  (void)json_object_dotset_string(JSON_DeviceConfig, "schema_version", "3.0.0");
...
static uint8_t _PnPLGetDeviceStatusJSON(char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  */
  ...
  (void)json_object_dotset_number(JSON_Device, "protocol_id", 1); /* 0: BLE, 1: serial, 2:libusb */
