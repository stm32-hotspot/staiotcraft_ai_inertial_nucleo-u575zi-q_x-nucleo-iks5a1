/**
  ******************************************************************************
  * @file    App_model_Ism6hg256x_Acc.c
  * @author  SRA
  * @brief   Ism6hg256x_Acc PnPL Components APIs
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2022 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file in
  * the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  *
  ******************************************************************************
  */

/**
  ******************************************************************************
  * This file has been auto generated from the following DTDL Component:
  * dtmi:appconfig:nucleo_u575zi_q:STAIOTCRAFT_AI_Inertial:sensors:ism6hg256x_acc;1
  *
  * Created by: DTDL2PnPL_cGen version 3.1.0-alpha.2
  *
  * WARNING! All changes made to this file will be lost if this is regenerated
  ******************************************************************************
  */

#include "App_model.h"

/* USER includes -------------------------------------------------------------*/

/* USER private function prototypes ------------------------------------------*/

/* USER defines --------------------------------------------------------------*/

/* ism6hg256x_ACC PnPL Component ---------------------------------------------*/
static SensorModel_t ism6hg256x_acc_model;
extern AppModel_t app_model;

uint8_t ism6hg256x_acc_comp_init(void)
{
  ism6hg256x_acc_model.comp_name = ism6hg256x_acc_get_key();

  uint16_t id;
  if (getNextSensorId(&id) != 0)
  {
    /* Sensor slot not available */
    return PNPL_BASE_ERROR_CODE;
  }
  addSensorToAppModel(id, &ism6hg256x_acc_model);

  ism6hg256x_acc_set_odr_initial_value(pnpl_ism6hg256x_acc_odr_hz7680);
  ism6hg256x_acc_set_fs_initial_value(pnpl_ism6hg256x_acc_fs_g16);
  /* USER Component initialization code */
  return PNPL_NO_ERROR_CODE;
}

char *ism6hg256x_acc_get_key(void)
{
  return "ism6hg256x_acc";
}


uint8_t ism6hg256x_acc_get_odr(pnpl_ism6hg256x_acc_odr_t *enum_id)
{
	float_t value;
	CUSTOM_MOTION_SENSOR_GetOutputDataRate(SENSOR_0,MOTION_ACCELERO,&value);
	if (value == 7.5f)
	{
		*enum_id = pnpl_ism6hg256x_acc_odr_hz7_5;
	}
	else if (value == 15.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_odr_hz15;
	}
	else if (value == 30.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_odr_hz30;
	}
	else if (value == 60.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_odr_hz60;
	}
	else if (value == 120.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_odr_hz120;
	}
	else if (value == 240.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_odr_hz240;
	}
	else if (value == 480.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_odr_hz480;
	}
	else if (value == 960.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_odr_hz960;
	}
	else if (value == 1920.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_odr_hz1920;
	}
	else if (value == 3840.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_odr_hz3840;
	}
	else if (value == 7680.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_odr_hz7680;
	}
	else
	{
		return 1; // Error code for unsupported ODR value
	}
	/* USER Code */
	return PNPL_NO_ERROR_CODE;
}

uint8_t ism6hg256x_acc_get_fs(pnpl_ism6hg256x_acc_fs_t *enum_id)
{
	int32_t value;
	CUSTOM_MOTION_SENSOR_GetFullScale(SENSOR_0,MOTION_ACCELERO,&value);
	if (value == 2.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_fs_g2;
	}
	else if (value == 4.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_fs_g4;
	}
	else if (value == 8.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_fs_g8;
	}
	else if (value == 16.0f)
	{
		*enum_id = pnpl_ism6hg256x_acc_fs_g16;
	}
	else
	{
		return 1; // Error code for unsupported FS value
	}
	/* USER Code */
	return PNPL_NO_ERROR_CODE;
}

uint8_t ism6hg256x_acc_get_stream_id(int8_t *value)
{
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t ism6hg256x_acc_get_ep_id(int8_t *value)
{
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t ism6hg256x_acc_set_odr_initial_value(pnpl_ism6hg256x_acc_odr_t enum_id){

	float_t value = enum_id;
	CUSTOM_MOTION_SENSOR_SetOutputDataRate(SENSOR_0,MOTION_ACCELERO,value);
	/* USER Code */
	return PNPL_NO_ERROR_CODE;
}

uint8_t ism6hg256x_acc_set_fs_initial_value(pnpl_ism6hg256x_acc_fs_t enum_id){

	int32_t value = enum_id;
	CUSTOM_MOTION_SENSOR_SetOutputDataRate(SENSOR_0,MOTION_ACCELERO,value);
	/* USER Code */
	return PNPL_NO_ERROR_CODE;
}

